...
// 2. SESSION & APP STATE
// ============================================
let currentOwner = ownerData[0]; // Logged in owner
...
function handleOwnerLogin(e) {
    ...
    // Simple mock auth matching against owner list
    const found = ownerData.find(o => o.email === email && o.password === password);
    if (found) {
        currentOwner = found;
        updateProfileDisplay();
        showToast('Selamat datang kembali, Mitra Owner! 👋');
        switchMainTab('dashboard');
    } else {
        // Fallback for demo convenience: auto-login anyway but create profile
        currentOwner = {
            id_owner: ownerData.length + 1,
            nama: "Mitra Jaya Rent",
            email: email,
            password: password,
            no_hp: "081250778888"
        };
        ownerData.push(currentOwner);
        updateProfileDisplay();
        showToast('Login Berhasil! (Akun Demo) ✅');
        switchMainTab('dashboard');
    }
}
...
function handleOwnerRegister(e) {
    ...
    const phone = document.getElementById('reg-no_hp').value;
    ...
    ownerData.push(currentOwner);
    updateProfileDisplay();
    showToast('Registrasi Berhasil! Selamat bermitra 🤝');
    switchMainTab('dashboard');
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Show splash screen, then route to login portal
    navigateTo('splash');
